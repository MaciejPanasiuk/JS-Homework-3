...
//1.4 Spendings by month
function SpendingsbyMonth(){
const Months=['January','February','March','April','May','June','July','August','September','October','November','December'];
const SpendingsPerMonth=[];
  for(m=0;m<12;m++){
  const Monthlyspending=financialData
  .filter((current)=>{if(Datesort(current.detailsOfPayent.date).getMonth()===m)
    { return current;}})// we get only objects that are within current month
    .map((currentvalue)=> +currentvalue.cost)
    .reduce((accu,element)=> accu + element,0);//we sum up all the spendings
    SpendingsPerMonth.push(Monthlyspending.toFixed(2));
}
const PresentableMonthlySpending = SpendingsPerMonth.map((Counter,index) =>(Months[index] + ': '+ Counter)).join(" ");
return PresentableMonthlySpending;  
}
//1.5 Spendings per day of the week

function SpendingsDayOfWeek(){
const Week=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday']
const WeekdaySpendingsummary=[];
for (d=0;d<7;d++){
  const WeekDayFinances=financialData
  .filter((current)=>{if(Datesort(current.detailsOfPayent.date).getDay()===d)
    { return current;}})// we get only objects that are within current day of the week
    .map((currentvalue)=> +currentvalue.cost)
    .reduce((accu,element)=> accu + element,0);//we sum up all the spendings
    WeekdaySpendingsummary.push(WeekDayFinances.toFixed(2));
}
const PresentableDailySpending = WeekdaySpendingsummary.map((Counter,index)=>(Week[index] + ': '+ Counter)).join(" ");
return PresentableDailySpending;
}
